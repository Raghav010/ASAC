...
class GetAudioVideoDataset(Dataset):
    ...
    def __init__(self, args, mode='train', transforms=None):
        ...
        if args.testset == 'flickr':
            ...
            self.image_path = args.data_path + '/frames/'
        else:
...
#  
    ...
    def __getitem__(self, idx):
        ...
        if self.args.testset == 'flickr':
            ...
            audiofilename = os.path.join(self.audio_path,file+'.wav')
...
#             filetmp = filetmp[:11]+'_'+str(int(filetmp[12:])*1000)+'_'+str((int(filetmp[12:])+10)*1000)
        ...
        if self.all_bboxes is not None:
            ...
            # if file doesnt exist in self.all_bboxes return dummy bboxes
            if file not in self.all_bboxes:
                bboxes['bboxes'] = bb
                bboxes['gt_map'] = bbox2gtmap(bb, self.args.testset)
                return frame,spectrogram,bboxes,file
            tmpbox = self.all_bboxes[file]
            bb[:len(tmpbox)] = torch.from_numpy(np.array(tmpbox))
            bboxes['bboxes'] = bb
            
            bboxes['gt_map'] = bbox2gtmap(self.all_bboxes[file], self.args.testset)
